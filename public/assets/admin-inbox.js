(function () {
  ...
  async function loadMessages(before, keepPolling) {
    ...
    try {
      const payload = await getJson(`${urls.messages}?dialog_id=${state.dialogId}&limit=40${beforePart}`);
    ...
    return `
      <article class="inbox-message ${Number(message.is_outgoing) ? 'outgoing' : 'incoming'}" data-message-id="${message.telegram_message_id}">
        <div class="inbox-bubble">
          ${Number(message.is_outgoing) ? '' : `<strong class="inbox-sender">${escapeHtml(message.sender_name || 'Telegram')}</strong>`}
  ...
  async function loadOlder() {
    ...
    try {
      const payload = await getJson(`${urls.messages}?dialog_id=${state.dialogId}&limit=40&before_message_id=${state.oldestMessageId}`);
